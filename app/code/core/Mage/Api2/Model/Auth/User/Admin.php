...
class Mage_Api2_Model_Auth_User_Admin extends Mage_Api2_Model_Auth_User_Abstract
{
    ...
    {
        return Mage::helper('api2')->__('Adminform');
    }
    ...
    public function getRole()
    {
        if (!$this->_role) {
            if (!$this->getUserId()) {
                throw new Exception('Adminform identifier is not set');
            }
            ...
            if (!$role->getId()) {
                throw new Exception('Adminform role not found');
            }
    ...
    {
        if ($this->_role) {
            throw new Exception('Adminform role has been already set');
        }
        $this->_role = $role;
